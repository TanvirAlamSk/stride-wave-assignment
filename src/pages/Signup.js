...
const Signup = () => {
    ...
    const { createUser, setUser, setLoader, googleSignIn } = useContext(authContext)
    ...
    const handelSignup = (event) => {
        ...
        createUser(email, password)
            .then((userCredential) => {
                ...
                fetch("https://recipe-easy-server.onrender.com/users", {
                    method: "POST",
                    headers: {
    ...
    return (
        <div className="h-full bg-gradient-to-br from-red-300 to-amber-300 w-full py-16 px-4">
            <div className="flex flex-col items-center justify-center">
                <img src={logo} alt='logo'></img>
                <p className='text-2xl font-extrabold text-white tracking-wider'>Recipe Easy</p>
                <div className="bg-white shadow rounded lg:w-1/3  md:w-1/2 w-full p-10 mt-16">
                    ...
                    <form onSubmit={handelSignup}>
                        <div>
                            ...
                            <input aria-label="enter your name" name='name' type="text" className="bg-gray-200 border rounded focus:outline-none text-xs font-medium leading-none text-gray-800 py-3 w-full pl-3 mt-2" required />
                        </div>
                        <div>
                            <label className="text-sm font-medium leading-none text-gray-800">Email</label>
                            <input aria-label="enter email address" name='email' type="email" className="bg-gray-200 border rounded focus:outline-none text-xs font-medium leading-none text-gray-800 py-3 w-full pl-3 mt-2" />
                        </div>
                        <div className="mt-6  w-full">
                            <label className="text-sm font-medium leading-none text-gray-800">Password</label>
                            <div className="relative flex items-center justify-center">
                                <input aria-label="enter Password" name='password' type={`${showPass ? "text" : "password"}`} className="bg-gray-200 border rounded focus:outline-none text-xs font-medium leading-none text-gray-800 py-3 w-full pl-3 mt-2" />
                                <div className="absolute right-0 mt-2 mr-3 cursor-pointer">
                                    <svg onClick={() => setShowPass(!showPass)} width={16} height={16} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path
